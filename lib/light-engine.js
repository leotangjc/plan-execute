...
const VALID_PHASES = ['compile', 'execute', 'done'];
function mdPath(slug) {
    return `${slug}.md`;
}
export function applyLight(ctx, _config = {}) {
    const config = {
        defaultSlug: 'plan',
        planDir: '.plan',
        autoTrigger: true,
    ...
    async function resolvePath(rel, exec) {
    ...
    /** 创建文件（原子：已存在则失败，绝不覆盖）。 */
    async function createFile(rel, content, exec) {
    ...
    async function writeMeta(slug, meta, exec) {
        ...
        emit('fs/observed', target, { kind: 'present', version: info.version }, exec);
        await waterfall('fs/write-intent', target, exec, () => undefined);
        const next = { ...meta, slug, updatedAt: new Date().toISOString() };
    ...
    function parseTasks(md) {
        ...
        for (const line of md.split('\n')) {
            ...
            if (m)
                tasks.push({ id: m[2], done: m[1].toLowerCase() === 'x', title: m[3] });
        }
        return tasks;
    }
    ...
    // ============ 动作 ============
    async function actionStart(slug, exec) {
    ...
    async function actionConfirm(slug, exec) {
        const meta = await readMeta(slug, exec);
    ...
    async function actionDeviation(slug, text, exec) {
    ...
    async function actionReport(slug, exec) {
        const meta = await readMeta(slug, exec);
        const phase = meta?.phase && VALID_PHASES.includes(meta.phase) ? meta.phase : 'compile';
    ...
    async function actionStop(slug, exec) {
        const meta = await readMeta(slug, exec);
    ...
    async function actionContinue(slug, exec, phaseArg) {
        const meta = await readMeta(slug, exec);
        if (!meta) {
            return { text: `没有计划「${slug}」，请先 action=start 新建。`, nextAction: 'start', phase: 'compile', slug };
        }
        // 支持 phase 参数回退（如 continue&phase=compile 重新编译）
        const phase = phaseArg && VALID_PHASES.includes(phaseArg) ? phaseArg : meta.phase || 'compile';
        if (phase === 'compile') {
    ...
    async function runCore(args, exec) {
        ...
        const phaseArg = typeof args.phase === 'string' ? args.phase : undefined;
        if (action === 'start')
            return await actionStart(slug, exec);
        if (action === 'confirm')
            return await actionConfirm(slug, exec);
        if (action === 'deviation')
            return await actionDeviation(slug, deviation || '', exec);
        if (action === 'report')
            return await actionReport(slug, exec);
        if (action === 'stop')
            return await actionStop(slug, exec);
        if (action === 'continue')
            return await actionContinue(slug, exec, phaseArg);
