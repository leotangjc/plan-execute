...
# 暂停与结束
- 用户说「结束 / 够了 / 结束拷问」→ 结束当前阶段，自动进入下一阶段；
- 用户说「暂停 / 停」→ 停留在当前阶段（不推进）：把未决问题写入 .grill 的 Unresolved Backlog，处理好后再说「结束」；
...
export function apply(ctx, _config = {}) {
    ...
    function hitWord(text, words) {
        ...
        return lower.some((w) => base === w.repeat(2) || base === w.repeat(3) || n === w.repeat(2) || n === w.repeat(3));
    }
