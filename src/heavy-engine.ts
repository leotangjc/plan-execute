...
const SELECTABLE_PHASES: readonly Phase[] = ['grill', 'compile', 'execute']

function planHead(slug: string, grillDir: string): string {
  return `# 执行计划: ${slug}\n\n> 输入来源: ${grillDir}/${slug}.md\n\n## 任务列表\n\n## 依赖图\n\n## 验收标准\n\n## 风险与假设\n\n## 未决项映射\n\n## 里程碑\n\n## 确认记录\n\n## 执行状态\n\n## 偏差记录\n`
}

export function applyHeavy(ctx: Context, _config: PlanExecuteConfig = {}) {
  const config = {
    defaultSlug: 'plan',
    grillDir: '.grill',
    planDir: '.plan',
  ...
  async function writeFile(rel: string, content: string, exec?: ExecCtx): Promise<void> {
    ...
    const outcome = await fs!.writeText(target, content, intent as FsWriteExpected | undefined, exec?.signal, policy)
  ...
  async function pointerFallback(exec?: ExecCtx): Promise<string> {
    ...
    if (current) {
      ...
      if (clean !== pickSlug(undefined)) {
        ...
        if (hasGrill || hasPlan) return clean
      }
    }
  ...
  async function writeMeta(slug: string, meta: Meta, exec?: ExecCtx): Promise<void> {
    ...
    const next: Meta = { ...prev, ...meta, updatedAt: new Date().toISOString(), schema: META_SCHEMA }
  ...
  async function buildReport(slug: string, exec?: ExecCtx): Promise<string> {
    ...
    const tasks: Record<string, unknown> =
      meta?.tasks && typeof meta.tasks === 'object' && !Array.isArray(meta.tasks) ? (meta.tasks as Record<string, unknown>) : {}
    const deviations = Array.isArray(meta?.deviations) ? (meta.deviations as string[]) : []
    ...
    for (const [id, rec] of Object.entries(tasks)) {
      if (!rec || typeof rec !== 'object' || typeof (rec as { status?: unknown }).status !== 'string') continue
      const status = (rec as { status: string }).status
      if (!VALID_STATUSES.includes(status)) continue
      counts[status] = (counts[status] ?? 0) + 1
      const reason = (rec as { reason?: string }).reason
      if (status === 'failed') failed.push(`${id}: ${reason || '(无描述)'}`)
      if (status === 'blocked') blocked.push(`${id}: ${reason || '(无描述)'}`)
    }
  ...
  async function recordStatus(slug: string, record: string, exec?: ExecCtx): Promise<string> {
    ...
    const tasks: Record<string, { status: string; reason?: string }> = { ...prevTasks }
    ...
    for (const r of Object.values(tasks)) {
      if (r && typeof r === 'object' && typeof r.status === 'string') c[r.status] = (c[r.status] ?? 0) + 1
    }
  ...
  async function runCore(args: Record<string, unknown>, exec: ExecCtx): Promise<StepResult> {
    ...
    if (action === 'report') {
      const meta = await readMeta(slug, exec)
      const ph = phase && SELECTABLE_PHASES.includes(phase as Phase) ? (phase as Phase) : meta && meta.phase ? meta.phase : await currentPhase(slug, exec)
      if (ph === 'execute') {
    ...
    if (section && content !== undefined) {
      ...
      const sectionPhase: Phase =
        phase && SELECTABLE_PHASES.includes(phase as Phase) ? (phase as Phase) : await currentPhase(slug, exec)
    ...
    let phaseNow: Phase
    if (phase && SELECTABLE_PHASES.includes(phase as Phase)) {
      phaseNow = phase as Phase
    ...
    const blocked = blockedMessage(phaseNow, action as import('./state-machine.js').Action)
  ...
  if (skills !== undefined) {
    const disposeSkill = skills.register({
      ...
      source: 'bundled',
      content: buildSkillContent('heavy', config.autoTrigger),
    })
