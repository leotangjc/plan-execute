/**
 * plan-workflow skill 正文（编排大脑）。独立模块，与引擎解耦。
 * 双模式：buildLightSkillContent（简化版）/ buildHeavySkillContent（完整版）。
 * 触发节按 autoTrigger 配置切换；light 任务行格式须与引擎 TASK_LINE 同步（- [x] T1: 标题）。
 */

export const HEAVY_TRIGGER_DEFAULT = `- 用户提出【需求模糊 / 多步骤 / 项目级】的新任务 → 默认进入本流程：先 plan_execute(action=start) 确认需求，再逐题拷问；
- 用户说出「计划实施」「拷问决策」「编译计划」「执行计划」「跑计划」→ 直接进入；
- 简单明确、一步可完成的请求（查资料、问问题、改个小文件）→ 不进入，直接做；
- 用户明确说「直接做 / 不用确认 / 跳过流程」→ 不进入，直接做。`

export const HEAVY_TRIGGER_PHRASE_ONLY = `- 仅当用户说出「计划实施」「拷问决策」「编译计划」「执行计划」「跑计划」时进入本流程；
- 其余任务直接执行；用户明确说「直接做 / 不用确认 / 跳过流程」时同样跳过。`

export function buildHeavySkillContent(autoTrigger: boolean): string {
  const trigger = autoTrigger ? HEAVY_TRIGGER_DEFAULT : HEAVY_TRIGGER_PHRASE_ONLY
  return `# Role
...
export const LIGHT_TRIGGER_DEFAULT = `- 用户提出【需求模糊 / 多步骤 / 项目级】的新任务 → 默认进入本流程：先 plan_execute(action=start) 新建计划，再写任务列表；
- 用户说出「计划实施」「编译计划」「执行计划」→ 直接进入；
- 简单明确、一步可完成的请求（查资料、问问题、改个小文件）→ 不进入，直接做；
- 用户明确说「直接做 / 不用确认 / 跳过流程」→ 不进入，直接做。`

export const LIGHT_TRIGGER_PHRASE_ONLY = `- 仅当用户说出「计划实施」「编译计划」「执行计划」时进入本流程；
- 其余任务直接执行；用户明确说「直接做 / 不用确认 / 跳过流程」时同样跳过。`

export function buildLightSkillContent(autoTrigger: boolean): string {
  const trigger = autoTrigger ? LIGHT_TRIGGER_DEFAULT : LIGHT_TRIGGER_PHRASE_ONLY
  return `# Role
你是「计划实施」流程的编排者兼主执行者。用户是唯一决策者，你永不替用户做决定。
...
/** 按模式选正文：light（简化）/ heavy（完整防御）。 */
export function buildSkillContent(mode: 'light' | 'heavy', autoTrigger: boolean): string {
  return mode === 'heavy' ? buildHeavySkillContent(autoTrigger) : buildLightSkillContent(autoTrigger)
}
