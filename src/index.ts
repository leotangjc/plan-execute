...
# 触发
用户说「计划实施」「拷问决策」「编译计划」「执行计划」「跑计划」时，加载本 skill 并驱动 plan_execute 工具。
...
export function apply(ctx: Context, _config: PlanExecuteConfig = {}) {
  ...
  function hitStopWord(text: string): boolean {
    const n = text.trim().replace(/[。．.！？!?，,、；;~～]+$/g, '').toLowerCase()
    if (STOP_WORDS.some((w) => n === w.toLowerCase())) return true
    if (/[了吧]$/.test(n)) {
      const m = n.slice(0, -1)
  ...
  const disposeTool = tools.register({
    name: 'plan_execute',
    description:
      '「计划实施」三合一编排引擎的记账本（状态机）：拷问决策(grill)→编译计划(compile)→执行验收(execute)。先用 skill 工具加载 plan-workflow 获取完整流程与提问策略，再驱动本工具。当用户说「计划实施」「拷问决策」「编译计划」「执行计划」「跑计划」时调用。多步驱动(action=start/answer/continue/report/stop)；主 agent 用 question 喂题、section+content 写计划字段、record/deviation 回写执行状态。',
    parameters: {
      type: 'object',
  ...
  if (skills !== undefined) {
    const disposeSkill = skills.register({
      ...
      whenToUse: '当用户说「计划实施」「拷问决策」「编译计划」「执行计划」「跑计划」时加载本 skill 并驱动 plan_execute 工具。',
      source: 'bundled',
