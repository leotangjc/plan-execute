...
import { ns, PlanExecuteSettingsSchema, type PlanExecuteSettings } from './settings.js'
...
export function apply(ctx: Context, _config: PlanExecuteConfig = {}) {
  // settings 服务由 host 平面提供，可能晚于本插件 apply 就绪。
  // 原实现先用 ctx.get('settings') 判断可用性——但 apply 阶段 settings 尚未提供时
  // 它必为 undefined，导致提前 dispatch 返回、schema 从未注册（设置卡片从未显示）。
  // 正确做法：无条件 ctx.inject(['settings']) 动态等待，服务就绪后注册并分发。
  ctx.inject(['settings'], (sctx) => {
    const settingsService = sctx.settings as {
...
/** 按 mode 分发到对应引擎。 */
function dispatch(ctx: Context, config: PlanExecuteConfig, mode: 'light' | 'heavy') {
  if (mode === 'heavy') {
