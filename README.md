...
└── .plan/
    ...
    └── <项目名>.meta.json   # 内部用：阶段指针 + 任务/偏差快照（别手改；报告唯一事实源）
```
...
- 「执行状态」「偏差记录」节是给人看的展示，里程碑报告只统计 `.plan/<项目名>.meta.json` 里的快照（record/deviation 写入）；手改展示节不影响报告；老计划（无快照）报告会显示暂无状态，用 record 重写即可；
- 工具参数 action=stop 与「暂停 / 停」不同：stop 在拷问/编译阶段只是停留（不推进、不锁死），在执行阶段安全收尾置 done——进度快照始终保留；
